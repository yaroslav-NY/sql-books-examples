...
ALTER TABLE products
  COMMENT = 'Contains one row for each type of product that is available for sale.';
...
ALTER TABLE offices
  COMMENT = 'Contains one row for each of the company’s five sales offices where the salespeople work.';
...
(
  ...
  FOREIGN KEY (rep_office) REFERENCES offices (office)
    ON DELETE SET NULL
    ON UPDATE CASCADE,
  FOREIGN KEY (manager) REFERENCES selesreps (empl_num)
    ON DELETE SET NULL
    ON UPDATE CASCADE
);
ALTER TABLE selesreps
  COMMENT = 'Contains one row for each of the company’s ten salespeople.';


ALTER TABLE offices
  ADD CONSTRAINT FOREIGN KEY (mgr) REFERENCES selesreps (empl_num)
  ON DELETE SET NULL
  ON UPDATE CASCADE;
...
(
  ...
  FOREIGN KEY (cust_rep) REFERENCES selesreps (empl_num)
    ON DELETE SET NULL
    ON UPDATE CASCADE
);
ALTER TABLE customers
  COMMENT = 'Contains one row for each of the company’s customers.';
...
(
  ...
  FOREIGN KEY (cust) REFERENCES customers (cust_num)
    ON DELETE RESTRICT
    ON UPDATE CASCADE,
  FOREIGN KEY (rep) REFERENCES selesreps (empl_num)
    ON DELETE SET NULL
    ON UPDATE CASCADE,
  FOREIGN KEY (mfr, product) REFERENCES products (mfr_id, prod_id)
    ON DELETE RESTRICT
    ON UPDATE CASCADE
);
ALTER TABLE orders
  COMMENT = 'Contains one row for each order placed by a customer. For simplicity, each order is assumed to be for a single product.';
