...
SELECT
  ...
  prod_id,
  description,
  (price * products.qty_on_hand) AS 'total cost'
FROM products;
...
SELECT
  name,
  quota,
  (quota + sales * .03) AS 'raised quota'
FROM selesreps;
...
SELECT
  ...
  title
FROM offices
  JOIN selesreps ON (offices.mgr = selesreps.empl_num);
...
SELECT
  order_date,
  company AS customer,
  name    AS 'customer’s salesperson',
  city    AS 'salesperson lives in'
FROM orders
  JOIN customers ON (orders.cust = customers.cust_num)
  JOIN selesreps ON (customers.cust_rep = selesreps.empl_num)
  JOIN offices ON (selesreps.rep_office = offices.office)
...
SELECT
  name,
  selesreps.sales AS 'empl. sales',
  city,
  offices.sales   AS 'office sales'
FROM selesreps
  JOIN offices ON (selesreps.rep_office = offices.office);
...
SELECT
  ...
  boss.name      AS 'boss name'
FROM selesreps
  JOIN selesreps AS boss
    ON (selesreps.manager = boss.empl_num);

-- List salespeople with a higher quota than their manager. --
SELECT
  selesreps.name AS employee,
  selesreps.quota   'empl. quota',
  boss.name      AS 'boss',
  boss.quota     AS 'boss quota'
FROM selesreps
  JOIN selesreps AS boss
    ON (selesreps.manager = boss.empl_num)
...
SELECT
  girls.name AS g_name,
  girls.city AS g_city,
  boys.name  AS b_name,
  boys.city  AS b_city
FROM girls
  INNER JOIN boys ON (girls.city = boys.city);
...
SELECT *
FROM girls
  RIGHT JOIN boys ON (girls.city = boys.city);
...
SELECT
  ...
  city
FROM selesreps
  LEFT JOIN offices ON (selesreps.rep_office = offices.office);
...
FROM
  (
      (girls
        LEFT JOIN parents ON (girls.name = parents.child)
        )
      LEFT JOIN boys ON (girls.city = boys.city))
...
SELECT
  TRUNCATE(AVG(quota), 2),
  TRUNCATE(AVG(sales), 2)
FROM selesreps;
...
SELECT
  ...
  SUM(amount)
FROM selesreps
  JOIN orders ON (selesreps.empl_num = orders.rep)
...
FROM selesreps
  JOIN customers ON selesreps.empl_num = customers.cust_rep
GROUP BY name;
...
SELECT
  ...
  SUM(amount)
FROM selesreps
  LEFT JOIN orders ON (selesreps.empl_num = orders.rep)
GROUP BY empl_num;
